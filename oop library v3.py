...
class Library:
    ...
    def reserve_book(self, book, member_id, due_date):
        ...
        if book.code not in self.user_data["reserved_books"]:
            ...
            result = f"The book '{book.title}' has already been reserved until {self.user_data['reserved_books'][book.code]['due_date']}."
    ...
    def return_book(self, book, member_id):
        if book.code in self.user_data["reserved_books"]:
            if self.user_data["reserved_books"][book.code]["member_id"] == member_id:
                self.user_data["reserved_books"].pop(book.code)
                book.check_in()
                result = f"The book '{book.code}' has been successfully returned."
            else:
                result = f"The book '{book.code}' has been reserved by another member."
        else:
            result = f"The book '{book.code}' has not been reserved by any member."
...
while True:
    ...
    selected_code = input("Which book would you like to reserve or return? (Enter the book code, 'q' to exit): ")
    operator = input("Which operator do you want? (reserve or return): ")
    ...
    if operator == "reserve":
        ...
        if selected_book:
            member_id = input("Library Member ID: ")
            # Return the book and show the result
            result = library.return_book(selected_book, member_id)
            print(result)
        else:
            print("Book not found.")
    else:
